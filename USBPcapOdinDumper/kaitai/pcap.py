...
class Pcap(KaitaiStruct):
    ...
    class Packet(KaitaiStruct):
        ...
        def _read(self):
            ...
            if _on == Pcap.Linktype.usb_linux:
                self._raw_body = self._io.read_bytes(self.incl_len)
                _io__raw_body = KaitaiStream(BytesIO(self._raw_body))
                self.body = usbmon.Usbmon(48, _io__raw_body)
            ...
            elif _on == Pcap.Linktype.usbpcap:
                self._raw_body = self._io.read_bytes(self.incl_len)
                _io__raw_body = KaitaiStream(BytesIO(self._raw_body))
                self.body = usbpcap.Usbpcap(_io__raw_body)
            elif _on == Pcap.Linktype.usb_linux_mmapped:
                self._raw_body = self._io.read_bytes(self.incl_len)
                _io__raw_body = KaitaiStream(BytesIO(self._raw_body))
                self.body = usbmon.Usbmon(64, _io__raw_body)
            else:
